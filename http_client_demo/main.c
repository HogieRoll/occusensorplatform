...
#define HOST_NAME       	"1-dot-occusensor.appspot.com" //"<host name>"
#define HOST_PORT           80
...
//!
//*****************************************************************************
static int HTTPPostMethod(HTTPCli_Handle httpClient, int OccStatus)
{
    bool moreFlags = 1;
    bool lastFlag = 1;
    char tmpBuf[4];
    ...
    HTTPCli_Field fields[4] = {
                                ...
                                {HTTPCli_FIELD_NAME_CONTENT_TYPE, "application/json"},/*Alex Change*/
                                {NULL, NULL}
                            };
    ...
    */
    ...
    if(OccStatus==true)
    {
    	sprintf((char *)tmpBuf, "%d", (sizeof(POST_DATA_OCCUPIED)-1));
    }
    else
    {
    	sprintf((char *)tmpBuf, "%d", (sizeof(POST_DATA_NOCCUPIED)-1));
    }
    ...
    */
    ...
    if(OccStatus==true)
    {
    	lRetVal = HTTPCli_sendRequestBody(httpClient, POST_DATA_OCCUPIED, (sizeof(POST_DATA_OCCUPIED)-1));
    }
    else
    {
    	lRetVal = HTTPCli_sendRequestBody(httpClient, POST_DATA_NOCCUPIED, (sizeof(POST_DATA_NOCCUPIED)-1));
    }
    if(lRetVal < 0)
    {
...
while(1)
{
    ...
    UART_PRINT("\n PIR Sensor Reading: %li\n\r", PIRPositiveReadings);
#endif /*PIR_SENSOR_ENABLE*/

#ifdef ULTRASONIC_SENSOR_ENABLE
    ...
    GPIOPinWrite(GPIOA0_BASE, 0x10, 0x0);
    //poll GPIOA0 Pin 58
    UltraSonicReading=0;int flag=0;
    int safetycounter=10000000;
    ...
    if(safetycounter==0)
    {
    	UART_PRINT("UltraSonic Sensor Failure\n\r");
    }
    else
    {
    	UART_PRINT("\nUltraSonic Sensor Reading: %li\n\r", UltraSonicReading);
    }

#endif /*ULTRASONIC_SENSOR_ENABLE*/
    int CurrentOccStatus=0;
    if(UltraSonicReading>5||PIRPositiveReadings>0)
    {
    	CurrentOccStatus=1;
    }
    else
